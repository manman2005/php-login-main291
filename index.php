...
$sql = "SELECT v.*, COUNT(DISTINCT vt.id) as vote_count
        ...
        GROUP BY v.vote_id, v.vote_name, v.date, v.start_time, v.end_time, v.created_at, v.status
...
<head>
    ...
    <style>
        ...
        .navbar-brand img { width: 40px; height: 40px; object-fit: cover; border: 2px solid white; margin-right: 10px; }
        ...
        .carousel { margin-bottom: 3rem; border-radius: 15px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.1);}
        .carousel-inner { height: 400px; border-radius: 15px; }
        .carousel-item { height: 400px; }
        .carousel-item img { width: 100%; height: 100%; object-fit: cover; }
        .page-header { background: linear-gradient(135deg, #0d6efd 0%, #0dcaf0 100%); color: white; padding: 3rem 0; margin-bottom: 2rem; text-align: center; }
        .voting-card { background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.1); margin-bottom: 2rem; transition: all 0.3s ease; }
        .voting-card:hover { transform: translateY(-5px); box-shadow: 0 8px 25px rgba(0,0,0,0.15);}
        .voting-header { background: linear-gradient(135deg, #0d6efd 0%, #0dcaf0 100%); color: white; padding: 1rem; }
        .voting-body { padding: 1.5rem; }
        .btn-vote { background: linear-gradient(135deg, #0d6efd 0%, #0dcaf0 100%); border: none; padding: 0.5rem 1.5rem; font-size: 1rem; font-weight: 500; border-radius: 50px; color: white; transition: all 0.3s ease; }
        .btn-vote:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(13, 110, 253, 0.4); color: white; }
        .status-badge { padding: 0.5rem 1rem; border-radius: 50px; font-size: 0.9rem; font-weight: 500; }
        .status-active { background-color: #28a745; color: white; }
        .status-upcoming { background-color: #ffc107; color: #000; }
        .status-ended { background-color: #dc3545; color: white; }
        .stats-card { background: white; border-radius: 15px; padding: 1.5rem; margin-bottom: 1rem; box-shadow: 0 5px 15px rgba(0,0,0,0.1); transition: all 0.3s ease; }
        .stats-card:hover { transform: translateY(-5px);}
        .stats-icon { width: 48px; height: 48px; background: linear-gradient(135deg, #0d6efd 0%, #0dcaf0 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.5rem; margin-bottom: 1rem; }
        .announcement-card { background: linear-gradient(135deg, #ffffff 0%, #f8f9fa 100%); border-left: 4px solid #0d6efd; padding: 1rem; margin-bottom: 1rem; border-radius: 8px; transition: all 0.3s ease; }
        .announcement-card:hover { transform: translateX(5px);}
        .search-box { position: relative; margin-bottom: 2rem; }
        .search-box input { padding-left: 3rem; border-radius: 50px; border: 2px solid #e0e0e0; transition: all 0.3s ease; }
        .search-box input:focus { border-color: #0d6efd; box-shadow: 0 0 0 0.2rem rgba(13, 110, 253, 0.25);}
        .search-icon { position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: #666; }
        .theme-switch { position: fixed; bottom: 2rem; right: 2rem; z-index: 1000; }
        .theme-btn { width: 50px; height: 50px; border-radius: 50%; background: linear-gradient(135deg, #0d6efd 0%, #0dcaf0 100%); color: white; border: none; box-shadow: 0 5px 15px rgba(0,0,0,0.2); transition: all 0.3s ease; }
                        ...
                        <div class="voting-card">
                            <div class="voting-header">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <?php
                                    // แสดงสถานะตามฟิลด์ status ในฐานข้อมูล
                                    if (isset($row['status'])) {
